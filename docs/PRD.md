...
| Hybrid search fused with RRF | `answer.py` runs dense vector search (top 15) and BM25 keyword search (top 15) independently, then fuses them with Reciprocal Rank Fusion, `k=60`, to a shortlist of 12 (wide-then-precise: catalog docs share most vocabulary, so stage-1 recall needs depth before the cross-encoder) | `evidence/rag/` logs with `--compare` output showing distinct BM25/Vector/RRF top-3 ids |
